...
pub fn init(path: PathBuf, level: LogLevel) {
    ...
    if let Err(error) = sink.open() {
        eprintln!("UsageDeck file log disabled: {error}");
    }
...
pub fn emit(level: LogLevel, tag: &str, arguments: fmt::Arguments<'_>) {
    ...
    let Ok(mut sink) = logger.sink.lock() else {
        eprintln!("UsageDeck file log disabled: lock unavailable");
        return;
    };
...
pub fn default_log_path() -> PathBuf {
    default_log_directory().join("UsageDeck.log")
}
...
fn default_log_directory() -> PathBuf {
    ...
    std::env::temp_dir().join("UsageDeck").join("logs")
}
...
mod tests {
    ...
    fn default_path_uses_the_platform_log_directory_and_advertised_file_name() {
        ...
        #[cfg(target_os = "macos")]
        assert!(path.to_string_lossy().contains("Library/Logs/UsageDeck"));
        #[cfg(target_os = "windows")]
        assert!(path.to_string_lossy().contains("UsageDeck\\logs"));
        #[cfg(all(unix, not(target_os = "macos")))]
        assert!(path.to_string_lossy().contains("usagedeck/logs"));
    }
    ...
    fn rotation_keeps_one_archive_and_trims_oversize_on_open() {
        ...
        assert!(!directory.path().join("UsageDeck.2.log").exists());
    ...
    fn sink_failure_disables_file_logging_without_panicking() {
        ...
        let mut sink = LogFile::new(blocked_parent.join("UsageDeck.log"), 200);
