...
const SOURCE: &str = include_str!("../../assets/usagedeck-tray.svg");
...
fn render_rgba(display_fraction: f64, remaining_fraction: f64) -> Vec<u8> {
    ...
    draw_paths(
        &mut pixmap,
        std::slice::from_ref(&paths.brand_mark),
        quota_color,
        transform,
    );
...
fn parse_gauge_paths() -> Result<GaugePaths, String> {
    ...
    require_element(&document, "usagedeck-tray")?;
    ...
    let brand_mark = parse_named_path(&document, "brand-mark", None)?;
    Ok(GaugePaths {
        track,
        fill,
        brand_mark,
...
mod tests {
    ...
    fn bundled_svg_exposes_the_expected_geometry() {
        ...
        assert!(paths.brand_mark.bounds().width() > 0.0);
    }
