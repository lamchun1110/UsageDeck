    ...
    fn update_internal(
        ...
        if expected_account_revision != Some(self.account_revision.load(Ordering::SeqCst)) {
            let active_provider_ids = self
                .active_account_identities
                .read()
                .map_err(|_| "UsageDeck account names are temporarily unavailable.".to_owned())?
                .keys()
                .cloned()
                .collect::<Vec<_>>();
        ...
        .map_err(|_| "UsageDeck account names could not be saved.".to_owned())?;
        self.storage
...
/// Rebuilds the stored provider option selections from the catalog.
///
/// Options a provider no longer declares are dropped, and any value that is not one of the
/// option's own choices falls back to its default. Every coherent option is materialized so the
/// settings screen and the providers always read the same concrete choice.
fn normalize_provider_options(catalog: &ProviderCatalog, settings: &mut AppSettings) {
    let mut normalized = BTreeMap::new();
    for definition in &catalog.providers {
        let stored = settings.provider_options.get(&definition.id);
        let resolved = definition
            .options
            .iter()
            .filter(|option| option.is_coherent())
            .map(|option| {
                let selected = option.resolve(
                    stored
                        .and_then(|options| options.get(&option.id))
                        .map(String::as_str),
                );
                (option.id.clone(), selected.to_owned())
            })
            .collect::<BTreeMap<_, _>>();
        if !resolved.is_empty() {
            normalized.insert(definition.id.clone(), resolved);
        }
    }
    crate::provider_options::publish(&normalized);
    settings.provider_options = normalized;
}
...
mod tests {
    ...
    use crate::{
        ...
        providers::{
            antigravity, claude, codex, cursor, kimi, openrouter, CredentialProbeResults,
    ...
    /// A registry holding the one provider that declares options, plus the fallback-enabled
    /// provider every registry requires.
    fn option_catalog() -> Arc<ProviderRegistry> {
        let providers = [claude::definition(), kimi::definition()]
            .into_iter()
            .map(|definition| Arc::new(CatalogProvider(definition)) as Arc<dyn UsageProvider>)
            .collect();
        Arc::new(ProviderRegistry::new(providers).unwrap())
    ...
    fn provider_options_materialize_their_default_choice() {
        ...
        let selected = settings.provider_options.get("kimi").unwrap();
        assert_eq!(
            selected.get("endpoint").map(String::as_str),
            Some("kimi.com")
        );
    }
    ...
    fn a_stored_provider_option_choice_is_preserved() {
        ...
        settings.provider_options.insert(
            "kimi".into(),
            BTreeMap::from([("endpoint".to_owned(), "kimi.ai".to_owned())]),
        );

        normalize(&registry, &mut settings, &HashSet::new());

        assert_eq!(
            settings.provider_options["kimi"]["endpoint"].as_str(),
            "kimi.ai"
        );
    }
    ...
    fn unknown_providers_options_and_values_are_discarded_on_load() {
        ...
        settings.provider_options.insert(
            "kimi".into(),
            BTreeMap::from([
                ("endpoint".to_owned(), "kimi.example".to_owned()),
                ("retired-option".to_owned(), "anything".to_owned()),
            ]),
        );
        settings.provider_options.insert(
            "retired-provider".into(),
            BTreeMap::from([("endpoint".to_owned(), "kimi.ai".to_owned())]),
        );

        normalize(&registry, &mut settings, &HashSet::new());

        assert!(!settings.provider_options.contains_key("retired-provider"));
        let selected = &settings.provider_options["kimi"];
        assert!(!selected.contains_key("retired-option"));
        assert_eq!(selected["endpoint"].as_str(), "kimi.com");
    }
