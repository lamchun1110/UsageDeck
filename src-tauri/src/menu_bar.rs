...
use crate::svg_path::parse_path_data;
...
fn render_text_strip(
    ...
    let width = (content_width + metrics.outer_padding * 2.0)
        .ceil()
        .max(1.0) as u32;
    // Strip width follows measured text and is only bounded by the OS; an
    // allocation failure must degrade to the fallback icon, not panic the
    // refresh loop.
    let mut pixmap = Pixmap::new(width, metrics.height)?;
