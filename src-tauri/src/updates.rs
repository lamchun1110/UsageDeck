...
const RELEASE_URL: &str = "https://github.com/lamchun1110/UsageDeck/releases/latest";
...
fn classify_updater_error(error: &UpdaterError, operation: &'static str) -> UpdateFailure {
    ...
    if matches!(error, UpdaterError::Reqwest(_) | UpdaterError::Network(_)) {
        return UpdateFailure::new(
            "network",
            format!("UsageDeck could not {operation} because the network request failed."),
    ...
    UpdateFailure::new(
        "update_failed",
        format!("UsageDeck could not {operation}."),
...
pub fn open_update_page() -> Result<(), String> {
    ...
    background_command(program)
        ...
        .spawn()
        .map(|_| ())
        .map_err(|error| format!("The UsageDeck download page could not be opened: {error}"))
}
