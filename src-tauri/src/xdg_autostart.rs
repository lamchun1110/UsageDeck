...
const DESKTOP_FILE: &str = "com.lamchun1110.usagedeck.desktop";
...
pub fn set_enabled(enabled: bool) -> Result<(), String> {
    ...
    let executable =
        std::env::current_exe().map_err(|_| "UsageDeck executable path could not be resolved.")?;
...
Exec={}\n\
Icon=com.lamchun1110.usagedeck\n\
Terminal=false\n\
StartupNotify=false\n\
...
mod tests {
    ...
    fn honors_absolute_xdg_config_home_and_falls_back_to_home() {
        assert_eq!(
            ...
            PathBuf::from("/custom/config/autostart/com.lamchun1110.usagedeck.desktop")
        );
        assert_eq!(
            autostart_path(
            ...
            .unwrap(),
            PathBuf::from("/home/user/.config/autostart/com.lamchun1110.usagedeck.desktop")
        );
    }

    #[test]
    fn desktop_entry_quotes_reserved_exec_characters() {
        let entry = desktop_entry(Path::new("/opt/Usage Deck/$test`bin"));
        assert!(entry.contains("Exec=\"/opt/Usage Deck/\\$test\\`bin\""));
    ...
    fn installs_and_removes_an_xdg_desktop_entry() {
        ...
        let path = directory.path().join("autostart/usagedeck.desktop");
        set_enabled_at(&path, Path::new("/opt/Usage Deck/usagedeck"), true).unwrap();
        let entry = std::fs::read_to_string(&path).unwrap();
        assert!(entry.contains("Exec=\"/opt/Usage Deck/usagedeck\""));
